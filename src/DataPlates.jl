...
"""
    DataPlates.plate(platename, geometry = 96)
    DataPlates.plate([plate1, plate2], geometry = 96)
...
"""
wells(rows, cols) = vec(string.(repeat(DataPlates.LETTERS[1:rows],outer = cols),lpad.(repeat(1:cols, inner=rows),2,"0")))
...
function n_row(w)
    pat = r"(\D)(\d+)"
    m = match(pat, w)
    findfirst(DataPlates.LETTERS .== m[1])
end
...
function merge_info(df::DataFrame)
    res = @transform(df, Q = DataPlates.Q.(:well, :geometry), well96 = DataPlates.well96.(:well, :geometry), well384 = DataPlates.well384.(:well, :geometry))
...
"""
    ...
    Eg DataPlates.printplate(su3, :Q)
"""
